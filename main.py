...
from utils import restore_param
...
loss_func = backpack.extend(loss_func)
...
def train(epoch):
    ...
    for batch_idx in range(steps):
        ...
        if args.private:
            ...
            with backpack.backpack(BatchGrad()):
                loss.backward()
            for p in net.parameters():
            ...
            theta_noise = torch.normal(
                0,
                noise_multiplier0 * args.clip0 / args.batchsize,
                size=clipped_theta.shape,
                device=clipped_theta.device
            )
            grad_noise = torch.normal(
                0,
                noise_multiplier1 * args.clip1 / args.batchsize,
            ...
            if logging:
                print(
                    'target grad norm: %.2f, noisy approximation norm: %.2f' % (
